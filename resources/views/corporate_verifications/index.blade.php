...
<style>
    ...
    .custom-badge.verified { background: rgba(34, 197, 94, 0.1); color: #22c55e; }
    .custom-badge.not_verified { background: rgba(245, 158, 11, 0.1); color: #f59e0b; }
    .custom-badge.declined { background: rgba(220, 38, 38, 0.1); color: #dc2626; }
    .custom-badge.resubmit { background: rgba(83, 86, 251, 0.1); color: #5356FB; }
...
<div class="container">
    <div class="row align-items-center mb-3">
        <div class="col">
            <h1 class="h4 mb-0" style="font-weight: 800; color: #1A1D2F;">Corporate Verifications</h1>
        </div>
    </div>
    ...
    <div class="card nftmax-filter-card">
        <div class="card-body p-3">
            <form id="filterForm" action="{{ route('corporate-verifications.index') }}" method="GET">
                <div class="row g-2 mb-3">
                    <div class="col-lg-4 col-md-6">
                        <label class="filter-group-label">Search</label>
                        <input type="text" name="search" class="form-control nftmax-filter-input" placeholder="Entity Name, Address, Type, ID..." value="{{ request('search') }}" autocomplete="off">
                    </div>
                    <div class="col-lg-3 col-md-6">
                        <label class="filter-group-label">Sort By</label>
                        <select name="sort" class="form-select nftmax-filter-input">
                            <option value="newest_to_oldest" {{ request('sort') == 'newest_to_oldest' ? 'selected' : '' }}>Newest First</option>
                            <option value="oldest_to_newest" {{ request('sort') == 'oldest_to_newest' ? 'selected' : '' }}>Oldest First</option>
                ...
                <div class="row g-2">
                    ...
                    <div class="col-auto">
                        <a href="{{ route('corporate-verifications.index') }}" class="btn btn-filter-reset">Reset Filters</a>
                    </div>
                </div>
            </form>
        </div>
    ...
    <div id="verifications-table-container">
        <div class="table-responsive">
            @include('corporate_verifications.table_partial')
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        const container = $('#verifications-table-container');
        ...
        function fetchVerifications() {
            ...
            const url = "{{ route('corporate-verifications.index') }}?" + formData;
