<table class="table nftmax-table" id="auctionsTable">
    <thead>
        <tr>
            <th>ID</th>
            <th>Product</th>
    ...
    <tbody>
        @foreach($auctions as $auction)
            ...
            <tr>
                ...
                <td style="color: #1A1D2F; font-weight: 800;">{{ number_format($auction->reserve_price) }}</td>
                <td>
                ...
                <td>
                    <div class="d-flex">
                        <a href="{{ route('auctions.show', $auction->id) }}" class="action-icon view" title="View"><i class="fas fa-eye"></i></a>
                        <a href="{{ route('auctions.edit', $auction->id) }}" class="action-icon edit" title="Edit"><i class="fas fa-edit"></i></a>
                        <form action="{{ route('auctions.destroy', $auction->id) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="action-icon delete" onclick="return confirm('Are you sure?')" title="Delete"><i class="fas fa-trash-alt"></i></button>
                        </form>
                    </div>
                </td>
            </tr>
...
<div class="d-flex justify-content-between align-items-center mt-3 mb-4 p-3" style="background: #fff; border-radius: 10px; border: 1px solid #f0f0f0;">
    <div class="small text-muted">Showing {{ $auctions->firstItem() }}-{{ $auctions->lastItem() }} of {{ $auctions->total() }} results</div>
    <div>
