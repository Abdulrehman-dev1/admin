...
<div class="container">
    <div class="row align-items-center mb-3">
        <div class="col">
            <h1 class="h4 mb-0" style="font-weight: 800; color: #1A1D2F;">Referrals Management</h1>
        </div>
    </div>
            ...
            <form id="filterForm" action="{{ route('referrals.index') }}" method="GET">
                <div class="row g-2 mb-3">
                    <div class="col-lg-4 col-md-6">
                        ...
                        <input type="text" name="search" class="form-control nftmax-filter-input" placeholder="ID, Name, Email, Code..." value="{{ request('search') }}" autocomplete="off">
                    </div>
                    <div class="col-lg-3 col-md-6">
                        <label class="filter-group-label">Sort By</label>
                        <select name="sort" class="form-select nftmax-filter-input">
                            <option value="newest_to_oldest" {{ request('sort') == 'newest_to_oldest' ? 'selected' : '' }}>Newest Users</option>
                            <option value="oldest_to_newest" {{ request('sort') == 'oldest_to_newest' ? 'selected' : '' }}>Oldest Users</option>
                            <option value="referrals_high_to_low" {{ request('sort') == 'referrals_high_to_low' ? 'selected' : '' }}>Referrals: High to Low</option>
                        </select>
                    </div>
                    <div class="col-lg-5 col-md-12">
                        <label class="filter-group-label">Join Date Range</label>
                ...
                <div class="row g-2">
                    ...
                    <div class="col-auto">
                        <a href="{{ route('referrals.index') }}" class="btn btn-filter-reset">Reset Filters</a>
                    </div>
                </div>
            </form>
        </div>
    ...
    <div id="referrals-table-container">
        <div class="table-responsive">
            @include('referral.table_partial')
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        const container = $('#referrals-table-container');
        ...
        function fetchReferrals() {
            ...
            const url = "{{ route('referrals.index') }}?" + formData;
