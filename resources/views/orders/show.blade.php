                        ...
                        @foreach($order->items as $item)
                            <tr>
                                <td>
                                    ...
                                    @if($item->variation)
                                        <br><small class="text-muted">Var: {{ $item->variation->name }}</small>
                                    @endif
                                </td>
                                <td>{{ $item->quantity }}</td>
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <p><strong>Payment Status:</strong>
                            <span
                                class="badge bg-{{ $order->payment_status === 'paid' || $order->payment_status === 'approved' ? 'success' : ($order->payment_status === 'failed' || $order->payment_status === 'declined' ? 'danger' : 'warning') }}">
                                {{ ucfirst($order->payment_status) }}
                            </span>
                        </p>
                        <p><strong>Order Status:</strong>
                            <span
                                class="badge bg-{{ $order->status === 'completed' ? 'success' : ($order->status === 'processing' ? 'info' : ($order->status === 'cancelled' ? 'danger' : 'warning')) }}">
                    ...
                    @if($order->receipt_image)
                        <div class="col-md-6">
                            ...
                            @endphp
                            <img src="{{ route('receipts.show', $receiptFilename) }}" alt="Receipt"
